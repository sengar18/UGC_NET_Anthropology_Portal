...
def _filter_lines(raw_text):
    ...
    for line in lines:
        ...
        if lower_line.startswith(("references", "suggested reading", "glossary", "let us sum up")):
...
def _rejoin_and_split(cleaned_lines):
...
def _format_paragraphs(joined_text):
    # Identify headers in the joined text (all caps usually)
    final_paras = []
    for para in joined_text.split('\n\n'):
        para = para.strip()
        if not para:
            continue
...
def clean_paragraph(raw_text):
    cleaned_lines = _filter_lines(raw_text)
    joined_text = _rejoin_and_split(cleaned_lines)
    return _format_paragraphs(joined_text)
